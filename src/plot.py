...
def draw_curve(args):
    ...
    with torch.no_grad():
        for x, _ in loader:
            ...
            if problem == "regression":
                logits = logits * config.preprocessor.y_std + config.preprocessor.y_mu                
            Logits.append(logits)
    Logits = torch.cat(Logits)
    Logits -= Logits.mean(dim=0)[None] # (dataset_size, n_comp, n_class)
    ...
    for i in tqdm.tqdm(range(config.model.n_feat)):
        ...
        logits = Logits[:, i, 0] # (dataset_size)
        logits = logits[unique_idx].detach().cpu()

        plt.clf()
        fig, ax = plt.subplots(figsize=(6,6))
        ax.plot(X_ord.iloc[unique_idx,i].astype(float), logits, label="logits", color=color, linewidth=3)
        ...
        plt.savefig(os.path.join(save_dir, X.columns[i]+".png"), bbox_inches='tight', dpi=1200)
